...
* 목차
  ...
  * [6. POP](#6-protocol-oriented-programming-pop)
  * [7. 그 밖에...](#7-else)
...
* body를 작성할 수 없는 protocol을 extension을 통해 body를 작성하여(초기구현하여) ***protocol에 기능을 부여***하고, 이를 사용하여 ***타입을 수평적으로 확장***할 수 있게 하는 기능
* 정의는 위와 같고 아무 생각없이 쓴다면, 그냥 쓸 수 있는 기술이고 패러다임이다
* 하지만 이유를 안다면 더욱 적극적으로 POP를 사용할 수 있기 때문에 내용을 요약해보고자 한다
  1. 하나의 타입은 하나만 상속받을 수 있었다
  2. 문제점에 대해서는 집합을 연상하면 좋다. A집합과 B집합, A∩B 집합에 대해 앞으로 편의상 A, B, X라 하겠다
  3. 만약에 상속을 받는데 X만 필요하다고 하면, 일단 여기서부터 문제가 발생한다. A-X나 B-X처럼 불필요한 내용을 상속받아야만 하기 때문이다
  4. 또 설령 상속을 받았다고 할지라도, 추후에 반대쪽의 내용이 더 필요해진다면? 예를 들어 A를 선택해서 상속받았더니 B의 내용이 필요해진다면? 이 상속관계를 바꾸어야 한다
  5. 그런데 이러한 내용이 A,B,C,D와 X,Y,Z까지 고려를 해야한다면? 머리가 터져버릴 수도 있다. 아예 원본으로부터 새로운 타입을 정의하는 게 속이 시원할 수도 있지만, 추후에는 더더더 머리가 터져버릴 수도 있다. A,B,C,D...에 이어서 E나 F까지 생겨버렸기 때문이다...🤯 끔찍하다
* POP는 위와 같은 교집합을 하나의 프로토콜로 정의하고, extension을 통해 default implementation을 지원함으로써 각각의 타입에서 교집합을 하나의 타입으로 정의하고, 이를 여러개 채택할 수 있게 하여 수평확장을 하게 도와준다
* 또한 이러한 POP는 상속을 할 수 없는 struct에 대해서 수평확장을 가능하게 함으로써, 참조타입이기 때문에 생산성이나 퍼포먼스를 따져야하는 class를 멀리하고 값타입을 더욱 적극적으로 쓸 수 있게 도와준다(사실은 이게 주목적이고 내가 한 이야기는 부수적인 결과다. 쉽게 설명하기 위해서...) 
